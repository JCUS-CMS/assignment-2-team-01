...
<?php
$swp_edd_archive_structure = get_option( 'swp_edd' );
if( is_plugin_active('easy-digital-downloads/easy-digital-downloads.php') && $swp_edd_archive_structure[0] == 'Enable' ){
 
$schema = edd_add_schema_microdata() ? 'itemscope itemtype="http://schema.org/Product" ' : ''; ?>

<div <?php echo $schema; ?>class="swp_edd_archive <?php echo esc_attr( apply_filters( 'edd_download_class', 'edd_download', get_the_ID(), $edd_download_shortcode_item_atts, $edd_download_shortcode_item_i ) ); ?>" id="edd_download_<?php the_ID(); ?>">
...
$swp_edd_archive_layout = get_theme_mod( 'swp_edd_archive_layout', array( 'image', 'title', 'description', 'cart' ) );
	if ( ! empty( $swp_edd_archive_layout ) && is_array( $swp_edd_archive_layout ) ) {
	    foreach ( $swp_edd_archive_layout as $swp_edd_archive_part ) {
	          get_template_part( 'edd_templates/edd-archive/shop-' . $swp_edd_archive_part);
	    }
}
